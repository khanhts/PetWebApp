...
<body class="bg-gray-100 p-6">
    <header class="bg-red-600 text-white text-center py-4 text-xl font-semibold mb-6">
        Quản lý tất cả đơn hàng (Admin)
    </header>

    <div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold mb-4">Danh sách tất cả đơn hàng</h2>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Người đặt (User)</th>
                        ...
                        <?php foreach ($orders as $order): ?>
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ...
                                    <span class="text-xs text-gray-500"><?php echo htmlspecialchars($order['customer_phone']); ?></span>
                                </td>
                                 <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <?php
                                        if (!empty($order['user_id'])) {
                                            echo htmlspecialchars($order['user_full_name'] ?? $order['user_username'] ?? 'User ID: ' . $order['user_id']);
                                        } else {
                                            echo '<span class="italic text-gray-400">Khách vãng lai</span>';
                                        }
                                    ?>
                                </td>
        ...
        <?php else: ?>
            <p class="text-gray-500">Chưa có đơn hàng nào trong hệ thống.</p>
        <?php endif; ?>
